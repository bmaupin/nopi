import { Blob } from 'buffer';

/**
 * CIA certificate chain
 */
export class CiaCertChain {
  private static readonly caCertificate = new Uint8Array([
    ,
  ]);

  private static readonly ticketCertificate = new Uint8Array([
    ,
  ]);

  private static readonly tmdCertificate = new Uint8Array([
    ,
  ]);

  blob: Blob;

  constructor() {
    // Create the blob right away so we only have to do it once
    this.blob = new Blob([
      CiaCertChain.caCertificate,
      CiaCertChain.ticketCertificate,
      CiaCertChain.tmdCertificate,
    ]);
  }

  public get size(): number {
    return this.blob.size;
  }

  public toBlob = () => {
    return this.blob;
  };
}
