import { Blob } from 'fetch-blob';

import forge from 'node-forge';

export class CiaTicket {
  private signatureParts = {
    // Hardcode the signature type (RSA_2048 SHA256)
    signatureType: new Uint8Array([0x00, 0x01, 0x00, 0x04]),
    signature: new Uint8Array(0x100),
    padding: new Uint8Array(0x3c),
  };

  private ticketHeaderParts = {
    // Hardcode the issuer since we've hardcoded the certs
    issuer: new Uint8Array([
      0x52, 0x6f, 0x6f, 0x74, 0x2d, 0x43, 0x41, 0x30, 0x30, 0x30, 0x30, 0x30,
      0x30, 0x30, 0x33, 0x2d, 0x58, 0x53, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30,
      0x30, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00,
    ]),
    eccPublicKey: new Uint8Array(0x3c),
    formatVersion: new Uint8Array([0x01]),
    // https://github.com/3DSGuy/Project_CTR/blob/319c4c6a24747a6def3c012312ed13a2f76bb369/makerom/cia.c#L170
    caCrlVersion: new Uint8Array(1),
    // https://github.com/3DSGuy/Project_CTR/blob/319c4c6a24747a6def3c012312ed13a2f76bb369/makerom/cia.c#L171
    signerCrlVersion: new Uint8Array(1),
    // TODO: the title key only seems to be encrypted if ciaset->content.encryptCia is true, otherwise it's just random
    // Last byte is padding
    // https://github.com/3DSGuy/Project_CTR/blob/319c4c6a24747a6def3c012312ed13a2f76bb369/makerom/cia.c#L188
    // titleKey: new Uint8Array([...utils.randomBytes(0x10), 0]),
    // TODO: test signature
    titleKey: new Uint8Array([
      0x83, 0xfe, 0x87, 0x7c, 0x0a, 0x38, 0x1e, 0xca, 0x1e, 0xc0, 0x65, 0x39,
      0xe5, 0x3a, 0x65, 0x97,
    ]),
    padding0: new Uint8Array(1),
    // Ticket ID always seems to be random
    // https://github.com/3DSGuy/Project_CTR/blob/319c4c6a24747a6def3c012312ed13a2f76bb369/makerom/cia.c#L179
    // ticketId: new Uint8Array([0x00, 0x04, ...utils.randomBytes(6)]),
    // TODO: test signature
    ticketId: new Uint8Array([0x00, 0x04, 0x5e, 0x41, 0xbe, 0xc6, 0xac, 0x3f]),
    // https://github.com/3DSGuy/Project_CTR/blob/319c4c6a24747a6def3c012312ed13a2f76bb369/makerom/user_settings.c#L114
    deviceId: new Uint8Array(4),
    // Create a random title ID
    // https://www.3dbrew.org/w/index.php?title=Titles&mobileaction=toggle_view_mobile
    titleId: new Uint8Array([
      // 0x00,
      // 0x04,
      // 0x00,
      // 0x00,
      // ...utils.randomBytes(3),
      // 0x00,
      // TODO: testing signature of hello world CIA
      0x00, 0x04, 0x00, 0x00, 0x0f, 0xf3, 0xff, 0x00,
    ]),
    padding1: new Uint8Array(2),
    // TODO: TicketVersion: 2.0.0 (2048)
    // The Ticket Title Version is generally the same as the title version stored in the Title Metadata. Although it doesn't have to match the TMD version to be valid.
    // https://www.3dbrew.org/wiki/Ticket
    ticketVersion: new Uint8Array(2),
    padding2: new Uint8Array(8),
    licenseType: new Uint8Array(1),
    keyId: new Uint8Array(1),
    padding3: new Uint8Array(0x2a),
    eShopAccountId: new Uint8Array(4),
    padding4: new Uint8Array(1),
    audit: new Uint8Array(1),
    padding5: new Uint8Array(0x42),
    limits: new Uint8Array(0x40),
  };

  private ticketContentIndexParts = {
    unknown0: new Uint8Array([0x00, 0x01, 0x00, 0x14]),
    // size of the content header itself, plus the size of each segment (0x84) times the number of segments :/
    // Hard-code the total size
    totalSize: new Uint8Array([0x00, 0x00, 0x00, 0xac]),
    unknown1: new Uint8Array([0x00, 0x00, 0x00, 0x14]),
    unknown2: new Uint8Array([0x00, 0x01, 0x00, 0x14]),
    unknown3: new Uint8Array([0x00, 0x00, 0x00, 0x00]),
    // Hard-code the header size (the size of this class itself)
    headerSize: new Uint8Array([0x00, 0x00, 0x00, 0x28]),
    segmentNum: new Uint8Array([0x00, 0x00, 0x00, 0x01]),
    segmentSize: new Uint8Array([0x00, 0x00, 0x00, 0x84]),
    segmentTotalSize: new Uint8Array([0x00, 0x00, 0x00, 0x84]),
    unknown4: new Uint8Array([0x00, 0x03, 0x00, 0x00]),
    level: new Uint8Array([0x00, 0x00, 0x00, 0x00]),
    index: new Uint8Array([
      ,
    ]),
  };

  private ticketKey = {
    modulus: new Uint8Array([
      ,
    ]),
    publicExponent: new Uint8Array([0x01, 0x00, 0x01]),
    privateExponent: new Uint8Array([
      ,
    ]),
  };

  constructor() {
    this.calculateSignature();
  }

  calculateSignature() {
    const ticketData = Uint8Array.from([
      ...CiaTicket.concatUint8Arrays(Object.values(this.ticketHeaderParts)),
      ...CiaTicket.concatUint8Arrays(
        Object.values(this.ticketContentIndexParts)
      ),
    ]);

    // https://stackoverflow.com/a/58301910/399105
    const messageDigest = forge.md.sha256.create();
    const buffer = String.fromCharCode.apply(null, ticketData);
    messageDigest.update(buffer);

    const privateKey = forge.pki.rsa.setPrivateKey(
      new forge.jsbn.BigInteger(Array.from(this.ticketKey.modulus)),
      new forge.jsbn.BigInteger(Array.from(this.ticketKey.publicExponent)),
      new forge.jsbn.BigInteger(Array.from(this.ticketKey.privateExponent))
    ) as forge.pki.rsa.PrivateKey;

    const signature = privateKey.sign(messageDigest);

    console.log('signature=', Buffer.from(signature, 'ascii'));

    this.signatureParts.signature.set(Buffer.from(signature, 'ascii'));
  }

  // https://stackoverflow.com/a/38089419/399105
  static concatUint8Arrays = (arrays: Uint8Array[]) => {
    return arrays.reduce((a, b) => [...a, ...b], []);
  };

  public toBlob = () => {
    return new Blob([
      ...Object.values(this.signatureParts),
      ...Object.values(this.ticketHeaderParts),
      ...Object.values(this.ticketContentIndexParts),
    ]);
  };
}
